...
- **默认启用防御模式** (`defaultEnableDefense`)：上传无密码的防御版本，利用已保存的 `ASS_PasswordCorrect` 区分合法用户与盗模者。Defense 层默认 `Inactive`，仅 `IsLocal && !PasswordCorrect` 时切到 `Active`；Lock 层与 Defense 层条件一致，各自控制 UI 遮罩与防御组件。不生成 GesturePassword/Countdown/Audio 层，`PARAM_TIME_UP` 不注册
...
- **静音警告音效** (`muteWarningSound`)：不生成警告音频对象与音频 Animator 层，与 `hideUI` 互补
