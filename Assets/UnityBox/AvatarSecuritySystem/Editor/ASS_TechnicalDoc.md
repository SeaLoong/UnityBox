...
- **Remote → Concealed**：`PasswordCorrect = false`（所有客户端，身体隐藏但无遮罩）
...
手势保持和容错通过**定长动画剪辑**实现（使用 `Obfuscator.DummyPath()` 生成 dummy curve 路径，混淆时替换为随机名）。Holding 状态使用 `gestureMaxHoldTime` 秒动画，确认退出点在 `holdTime/maxHoldTime` 比例处。比对条件使用 avatar-hash 噪声扰动（noiseLow/noiseHigh 0/1 随机），攻击者无法确定精确手势值。
