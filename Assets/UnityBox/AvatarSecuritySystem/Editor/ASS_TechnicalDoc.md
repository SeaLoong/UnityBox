...
- **本地/远端分离**：防御(Countdown+Defense)仅本地触发；Lock层:远端看到身体隐藏(Concealed)但无遮罩,仅穿戴者本地看到全屏遮罩(Locked)
...
Avatar 加载
│
├─ [远端玩家] Remote → Concealed（身体隐藏，无遮罩）
│   远端观众只看到隐藏的身体，不会看到全屏遮罩方块
│   ┌─────────────────────────────┐
...
│   │
│   ├─ Remote → Concealed（全客户端：身体隐藏）
│   │   └─ Concealed → Locked（仅本地：加全屏遮罩）
│   │
│   ├─ Locked 状态
...
┌──────────┐  !Password  ┌────────────┐                      ↓
│  Remote  │───────────→│ Concealed  │              ┌──────────┐
│(默认状态) │←───────────│(藏体无遮罩) │──Password──→│ Unlocked │
└──────────┘  !Password  └────────────┘    Correct   │(恢复显示) │
    ↑                                                  │          │
    └────────────!PasswordCorrect──────────────────────┘          │
                                                                  │
    └────────────PasswordCorrect──────────────────────────────────┘
...
- **Remote → Concealed**：`PasswordCorrect = false`（**所有客户端**，身体隐藏但无遮罩。v0.5.0 移除了 IsLocal 条件）
- **Remote → Unlocked**：`PasswordCorrect = true`（所有玩家都能进入解锁状态）
- **Concealed → Locked**：`IsLocal = true` 且 `PasswordCorrect = false`（仅穿戴者，加上遮罩）
- **Concealed → Unlocked**：`PasswordCorrect = true`
- **Locked → Unlocked**：`PasswordCorrect = true`
- **Unlocked → Remote**：`PasswordCorrect = false`（密码被重置，回到初始态再分发到 Concealed）

> v0.5.0 已移除 `Locked → Locked` 自循环（`hasExitTime=true, exitTime=0` 导致 writeDefaultValues 每帧多次重置）。状态 clip 播完后最后一帧的值持续保留，无需自循环维持。

#### 4.1.2 锁定动画 (CreateLockClip)

- **启用**: `GO_OVERLAY` (全屏 Shader 覆盖)、`GO_AUDIO_WARNING`、`GO_AUDIO_Success`
- **禁用**: `GO_DEFENSE_ROOT` (防御根对象)
- **隐藏 Avatar**: 当 `disableRootChildren = true` 时，禁用所有非 ASS 根子对象 (`m_IsActive = 0` + `localScale = 0` 双重保护)

#### 4.1.3 隐藏动画 (CreateConcealedClip)

- **禁用**: `GO_OVERLAY`、`GO_DEFENSE_ROOT`
- **隐藏 Avatar**: 与 Locked 相同（`m_IsActive = 0` + `localScale = 0`），但不显示遮罩
- 远端观众停留在此状态，看到身体隐藏但无大白方块

#### 4.1.4 解锁动画 (CreateUnlockClip)
...
#### 4.1.5 Remote 动画 (CreateRemoteClip)

- **禁用**: `GO_OVERLAY`、`GO_DEFENSE_ROOT`
- **WD Off**: 显式恢复所有根子对象
- 用途：初始状态，随后所有客户端根据 PasswordCorrect 分流到 Concealed 或 Unlocked
...
> **v0.5.0 变更**：
> - 所有手势条件改用 `Greater+Less` 配对（替代旧版 `Equals`/`NotEqual`），防逆向
> - Confirmed 状态 clip 时长 = `gestureMaxHoldTime`，超时回到 Wait
> - 已移除 `confirmedStay` 自循环——保持确认手势会走错误路径进入 ErrorTolerance
> - 已移除首位密码手势重启转换——误触时 0.3s 容错窗口足以纠正，超时回到 Wait 后自然触发
> - 所有哈希噪声（`AddGestureConditions`/`AddErrorTransitions`）已移除，改用精确手势值比对
...
- **v0.5.0 移除重启转换**: 不再有首位密码手势→Step_1_Holding 的快捷路径，误触首位密码手势自然走 ErrorTolerance 窗口
...
| 参数                | 类型  | 默认值 | 范围   | 说明                                        |
| ------------------- | ----- | ------ | ------ | ------------------------------------------- |
| `countdownDuration` | float | 30     | 10-30 | 总倒计时时间（秒）（v0.4.0 调整）                          |
...
| `gestureMaxHoldTime`    | float | 3      | 1-10    | 手势最大保持时间（秒），超过则输入重置（v0.4.0） |
...
### 10.6 VRCFury IsLocal 参数类型兼容（v0.3.1）
...
### v0.5.1 — 假状态与参数生成全面随机化 (2026-06-28)

**假状态生成改进**：

- 所有假状态过渡条件混合 `Equals` 和 `Greater+Less` 配对（与真实代码格式一致）
- 每层使用独立 seed 偏移 → 各层假状态个数/位置/连接/条件完全不同
- Float/Int/Bool 三种参数类型混合使用
- exitTime 使用散列伪随机值替代等差序列
- `writeDefaultValues` 随机 true/false
- 部分假状态没有回到 default 的出口（增加拓扑多样性）

**诱饵参数池扩展**：

- 新增 Int 类型参数 (`_PolyCount`, `_LODLevel`, `_FrameIdx` 等)
- 新增 Avatar 风格参数 (`_GestureWt`, `_PoseBlend`, `_IkWeight`, `_LipSyncV` 等)
- 总候选数从 28 增至 45

### v0.5.0 — 混淆引擎与手势系统重构 (2026-06-28)

**手势密码系统重构**：

- 移除所有哈希噪声（`AddGestureConditions`/`AddErrorTransitions`/`_avatarHash`）
- 所有手势条件改用 `Greater+Less` 配对（`Greater, N-1 + Less, N+1` 等价于 `Equals, N`）
- 错误条件改用 `Less, N + Greater, N` 配对（等价于 `NotEqual, N`）
- Confirmed 状态 clip 时长改为 `gestureMaxHoldTime`，添加 `exitTime=1.0` 超时到 Wait
- 移除 `confirmedStay` 自循环（自循环会重置 Confirmed clip 计时器，使超时永不触发）
- 移除首位密码手势重启转换（Confirmed 和 ErrorTolerance 中的 restart 逻辑）
- 默认密码从 `[1, 7, 2, 4]` 改为 `[1, 2, 3, 4]`
- HoldTimeout (exitTime=1.0) 保留作为安全兜底

**Lock 层调整**：

- 移除 `Locked → Locked` 自循环（`hasExitTime=true, exitTime=0` 导致 writeDefaultValues 每帧多次重置）
- `Remote → Concealed` 条件从 `IsLocal && !PasswordCorrect` 改为 `!PasswordCorrect`（所有客户端生效）
- 新增 `Concealed` 中间状态（远端看到身体隐藏但无全屏遮罩）
- `Concealed → Locked` 保留 `IsLocal`（仅穿戴者看到全屏遮罩）

**合并其他 v0.4.x 变更**：

- 字段重命名：`hideUI` → `hideOverlay` → `disableOverlay`（带 FormerlySerializedAs 兼容）
- 字段重命名：`muteWarningSound` → `disableWarningSound`
- 常量重命名：`GO_UI` → `GO_OVERLAY`，`UI_OVERLAY_NAME` → `OVERLAY_MESH_NAME`
- 文件重命名：`Shaders/UI.shader` → `Shaders/Overlay.shader`
- Shader 内名：`UnityBox/ASS_UI` → `UnityBox/ASS_Overlay`
- I18n 键：`advanced.hide_ui` → `advanced.disable_overlay`
- 文案优化：tooltip 手动换行符、简繁中文统一

**混淆引擎集成**：

- `Obfuscator.cs`：确定性 FNV-1a 哈希命名 + 4 种格式变体
- `Constants.ApplyObfuscation()`：构建时动态替换所有静态名称
- Decoy 参数池（45 候选，随机选 3-5）
- Decoy 层（10 候选，随机选 1）
- Decoy 状态（每真层注入 5-8）
- Shader 副本混淆
...
### v0.4.6 — Bug 修复与混淆增强 (2026-06-28)

- 修复混淆启用时倒计时进度条路径不匹配
- Shader 内部 Pass 名处理、名称池扩展
- Lock 层新增 Concealed 状态
- 文档全面同步
...
- **静音警告音效** (`disableWarningSound`)：不生成警告音频对象与音频 Animator 层，与 `disableOverlay` 互补
...
- 修复混淆启用时倒计时进度条路径不匹配
- 移除 `RegisterASSParameters` 中重复的 Debug.Log
- 修复 Shader 内部 Pass 名仍含 `ASS_/UB_` 的问题
- 修复 Shader 混淆的正则表达式破坏 Unity 标准 Pass 名
- 移除 `OnValidate` 中对 `InitializePasswordIfNeeded` 的意外调用
- 修复 Inspector tooltip key
- 移除 `999999` 魔法数字，改用 Bool guard + `Greater 0.5f` fallback
- 扩大名称池规模，增强混淆多样性
- 假状态注入数量从 2-4 增至 5-8，使用网格式拓扑
- 恢复 GesturePassword 自循环
- DummyPath 改为 `__internal_dummy_anim__`（非混淆）/ 随机名（混淆）
- Overlay Mesh bounds 从 2 增至 200
- Shader 内部声明替换确保 AssetBundle 中 Shader 名无 `UnityBox/` 前缀
- Lock 层新增 Concealed 状态
- 文档全面同步至代码实现
...
- **假状态注入** (`enableDecoyStates`, 默认 true)：每个真层注入 5-8 个假状态
...
在每个真实 ASS 层的 StateMachine 中注入 5-8 个额外假状态：
...
| `enableDecoyStates` | true | 在每个真层中注入 5-8 个假状态 |
