...
- **无 NDMF**（仅 VRCFury 或纯 VRCSDK）：`Editor/NDMF` 子程序集不参与编译，`Processor` 作为 `IVRCSDKPreprocessAvatarCallback` 以固定的 `callbackOrder = -1024` 执行 ASS 主功能层生成（与 VRCFury 自身的 `VrcfRemoveEditorOnlyObjectsHook` 相同，但两者处理内容互不影响，相对顺序不影响结果）；Playable 混淆由独立的 `PlayableObfuscationProcessor` 在尾部阶段执行
...
int.MaxValue-100 : VRCFury ParameterCompressorHook
 int.MaxValue-1 : ★ ASS PlayableObfuscationProcessor（仅 Playable 混淆）
       MAX   : VRCFury Cleanup
...
存在 NDMF 时，ASS 统一在 **NDMF 自身的 BuildPhase.PlatformFinish 阶段**（在 `Optimizing` 之后，仍在虚拟/克隆状态、`context.Finish()` 提交之前）生成 ASS 功能层；playable controller 的混淆单独延后到 `callbackOrder = int.MaxValue - 1` 的尾部步骤。为兼容 lilycalinventory 的菜单/参数生成流程，ASS 在 NDMF 中显式设置为 `AfterPlugin("jp.lilxyzw.lilycalinventory")`。
两种场景下，VRCFury 参数压缩（`int.MaxValue - 100`）都晚于 ASS 主功能层生成，因此 ASS 参数会被正确识别；Playable 混淆则进一步后置到参数压缩之后执行。
...
│   ├─ PlayableObfuscationProcessor.cs                  # Playable 混淆后置处理器（callbackOrder = int.MaxValue - 1）
...
- **未安装 NDMF**：`Editor/NDMF` 子程序集不参与编译，`Processor` 使用固定的 `callbackOrder = -1024`，在 VRCFury 主处理（`-10000`）之后注入（与 VRCFury 自身的 `RemoveEditorOnlyObjects` 相同 `callbackOrder`，但两者处理内容互不影响，相对顺序不影响结果），并自行复制 Playable 层控制器到 Generated 目录；Playable 混淆由独立回调在尾部执行。
...
无论哪种模式，VRCFury 的参数压缩（ParameterCompressorHook）都在极后（`int.MaxValue - 100`）执行，ASS 新增参数会被正确识别和压缩；Playable 混淆由 `PlayableObfuscationProcessor` 在 `int.MaxValue - 1` 执行。
